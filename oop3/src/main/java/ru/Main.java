...
public class Main {
    ...
    private static void findElectricGuitarsByEffect() {
        ...
        try (Session session = GenericDAO.getSessionFactory().openSession()) {
            // Создаем CriteriaBuilder для построения запроса
            CriteriaBuilder cb = session.getCriteriaBuilder();
            CriteriaQuery<Long> cq = cb.createQuery(Long.class);
            Root<ElectricGuitar> root = cq.from(ElectricGuitar.class);
            Join<ElectricGuitar, Effect> effectJoin = root.join("effects");

            // Условие: эффект с заданным именем
            cq.select(root.get("id")).where(cb.equal(effectJoin.get("effectName"), effectName));

            // Получаем список ID гитар
            List<Long> guitarIds = session.createQuery(cq).getResultList();

            if (guitarIds.isEmpty()) {
                ...
                // Получаем полные объекты гитар по ID и выводим информацию
